...
public interface MongoGenDao<T> {
     ...
     * @return 结果信息
     */
    UpdateResult updateInsert(Query query, Update update);
     ...
     * @return 排序order list
     */
    static List<Order> processSort(SortField[] sort) {
        List<Order> order = new ArrayList<>();
        for (SortField sortField : sort) {
            String field = sortField.getField();
            Order entity = new Order(Direction.ASC.name().equals(sortField.getSort().name()) ? Direction.ASC : Direction.DESC, field);
            order.add(entity);
        }
        return order;
    }
     ...
     *
     * @param query          查询条件
     * @param includedFields 需要返回的属性字段
     * @return 查询结果
     */
    List<T> shrinkQuery(Query query, String... includedFields);
